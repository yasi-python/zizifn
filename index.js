...
const Config = {
  ...
  /**
   * @param {{ PROXYIP: string; UUID: any; SCAMALYTICS_USERNAME: any; SCAMALYTICS_API_KEY: any; SCAMALYTICS_BASEURL: any; SOCKS5: any; SOCKS5_RELAY: string; DOH_UPSTREAM_URL: string; DB: any; KV: any; ADMIN_KEY: any; }} env
   */
  fromEnv(env) {
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // Extract UUID from pathname for user-facing pages
    const pathSegments = url.pathname.split('/').filter(Boolean);
    const potentialUUID = pathSegments[0];
    
    if (isValidUUID(potentialUUID)) {
        const userIsValid = await isUserValid(potentialUUID, cfg);
        if (!userIsValid) {
            return new Response('Invalid or expired user UUID.', { status: 403 });
        }
        
        if (pathSegments.length === 1) { // Path is /{UUID}
            return handleConfigPage(potentialUUID, url.hostname, cfg.proxyAddress);
        }
        if (pathSegments[0] === 'xray' && pathSegments[1] && isValidUUID(pathSegments[1])) { // Path is /xray/{UUID}
            return handleIpSubscription('xray', pathSegments[1], url.hostname);
        }
        if (pathSegments[0] === 'sb' && pathSegments[1] && isValidUUID(pathSegments[1])) { // Path is /sb/{UUID}
            return handleIpSubscription('sb', pathSegments[1], url.hostname);
        }
    }
    ...
    // Fallback for old subscription links using env.UUID
    const mainUUIDs = cfg.userID.split(',');
    if (url.pathname.startsWith(`/xray/${mainUUIDs[0].trim()}`))
      return handleIpSubscription('xray', mainUUIDs[0].trim(), url.hostname);
    if (url.pathname.startsWith(`/sb/${mainUUIDs[0].trim()}`))
      return handleIpSubscription('sb', mainUUIDs[0].trim(), url.hostname);
    if (url.pathname.startsWith(`/${mainUUIDs[0].trim()}`))
      return handleConfigPage(mainUUIDs[0].trim(), url.hostname, cfg.proxyAddress);
...
/**
 * Handles DNS-over-HTTPS (DoH) proxy requests.
 * @param {Request} request
 * @param {object} config
 * @returns {Promise<Response>}
 */
async function handleDoHProxy(request, config) {
    const dohContentType = 'application/dns-message';
    const { search, method, headers, body } = request;
    const dohUrl = new URL(config.dohUpstreamUrl);
    dohUrl.search = search;
    ...
    try {
        ...
        return new Response('DoH proxy failed: ' + e.message, { status: 502 });
    }
}
...
/**
 * Validates a user UUID against KV cache and D1 database.
 * @param {string} uuid
 * @param {object} config
...
async function isUserValid(uuid, config) {
    if (!config.db) {
        // Fallback to static UUID check if DB is not configured
        const staticUUIDs = (config.userID || '').split(',').map(u => u.trim());
        return staticUUIDs.includes(uuid);
    }
    ...
    try {
        const stmt = config.db.prepare(
            'SELECT status, expiration FROM users WHERE uuid = ?1'
        );
        ...
        if (config.kv) {
            await config.kv.put(cacheKey, isValid ? 'valid' : 'invalid', {
                expirationTtl: CONST.KV_CACHE_TTL,
            });
        }
        return isValid;
    } catch (e) {
...
/**
 * Handles TCP outbound logic with direct connection and fallback to proxy.
 ...
 * @param {{ (info: any, event: any): void; }} log
 * @param {object} config
 */
async function HandleTCPOutBound(
  ...
  async function connectAndPipe(address, port, isProxy) {
    let tcpSocket;
    
    if (config.enableSocks) {
        log(`Connecting to SOCKS5 proxy: ${config.socks5Address}`);
  ...
  try {
    // 1. First attempt: Direct connection (or SOCKS5 if enabled and not in relay mode)
    ...
    // 2. Second attempt: Fallback to PROXYIP (if configured and not SOCKS)
    if (config.proxyIP && !config.enableSocks) {
      log(`Falling back to proxy: ${config.proxyIP}:${config.proxyPort}`);
      try {
        await connectAndPipe(config.proxyIP, parseInt(config.proxyPort, 10), true);
      } catch (proxyError) {
        log(`Fallback proxy connection failed: ${proxyError.message}`);
        throw proxyError; // Both failed, rethrow
      }
    } else {
 ...
 * @param {WebSocket} webSocket
 * @param {string | Uint8Array | ArrayBuffer | ArrayBufferView | Blob | null} protocolResponseHeader
 * @param {(() => Promise<void>) | null} retry
 * @param {{ (info: any, event: any): void; }} log
 */
async function RemoteSocketToWS(remoteSocket, webSocket, protocolResponseHeader, retry, log) {
  let hasIncomingData = false;
  let responseHeader = protocolResponseHeader;
  try {
 ...
 * @param {object} config
 */
async function handleAdminRoutes(request, config) {
    const url = new URL(request.url);
...
async function handleAdminApi(request, config) {
    const { adminKey, db, kv } = config;
    if (!adminKey) {
        return new Response(JSON.stringify({ error: 'ADMIN_KEY is not set' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
    }
    if (!db) {
        return new Response(JSON.stringify({ error: 'D1 database (DB) is not bound' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    try {
        if (path === '/admin/api/users' && request.method === 'GET') {
            const { results } = await db.prepare("SELECT id, uuid, expiration, status, note, created_at FROM users ORDER BY created_at DESC").all();
            return new Response(JSON.stringify(results), { headers: { 'Content-Type': 'application/json' } });
        }
        
        if (path === '/admin/api/users' && request.method === 'POST') {
            const { expiration_days, note } = await request.json();
            const newUUID = crypto.randomUUID();
            const id = newUUID.substring(0, 8); // Simple short ID
            const expirationTimestamp = Math.floor(Date.now() / 1000) + (parseInt(expiration_days, 10) * 86400);

            await db.prepare(
                "INSERT INTO users (id, uuid, expiration, note) VALUES (?1, ?2, ?3, ?4)"
            ).bind(id, newUUID, expirationTimestamp, note || '').run();
            
            return new Response(JSON.stringify({ success: true, uuid: newUUID }), { status: 201, headers: { 'Content-Type': 'application/json' } });
        }

        if (path.match(/^\/admin\/api\/users\/[a-zA-Z0-9-]+$/) && request.method === 'DELETE') {
            const uuidToDelete = path.split('/').pop();
            const { success } = await db.prepare("DELETE FROM users WHERE uuid = ?1").bind(uuidToDelete).run();
            if (success) {
                if (kv) await kv.delete(`uuid-status:${uuidToDelete}`);
                return new Response(JSON.stringify({ success: true }), { headers: { 'Content-Type': 'application/json' } });
            }
            return new Response(JSON.stringify({ error: 'User not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
        }
    } catch (e) {
        return new Response(JSON.stringify({ error: e.message, cause: e.cause?.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
    }
...
<head>
    ...
    <style>
        :root { --background-primary: #2a2421; --background-secondary: #35302c; --border-color: #5a4f45; --text-primary: #e5dfd6; --accent-primary: #be9b7b; --status-error: #e05d44; }
        body { font-family: sans-serif; background-color: var(--background-primary); color: var(--text-primary); display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; }
        .login-container { background-color: var(--background-secondary); padding: 2rem; border-radius: 8px; border: 1px solid var(--border-color); box-shadow: 0 4px 15px rgba(0,0,0,0.3); text-align: center; }
        h1 { margin-bottom: 1.5rem; }
        input { width: 100%; padding: 0.75rem; margin-bottom: 1rem; border-radius: 4px; border: 1px solid var(--border-color); background: var(--background-primary); color: var(--text-primary); font-size: 1rem; }
        button { padding: 0.75rem 1.5rem; border: none; border-radius: 4px; background-color: var(--accent-primary); color: #2a2421; font-size: 1rem; cursor: pointer; }
        .error { color: var(--status-error); margin-top: 1rem; }
    </style>
</head>
<body>
...
<head>
    ...
    <style>
        :root { --background-primary: #2a2421; --background-secondary: #35302c; --background-tertiary: #413b35; --border-color: #5a4f45; --text-primary: #e5dfd6; --text-secondary: #b3a89d; --accent-primary: #be9b7b; --status-error: #e05d44; }
        body { font-family: sans-serif; background-color: var(--background-primary); color: var(--text-primary); margin: 0; padding: 2rem; }
        .container { max-width: 900px; margin: auto; }
        h1, h2 { color: var(--accent-primary); border-bottom: 1px solid var(--border-color); padding-bottom: 0.5rem; }
        .card { background-color: var(--background-secondary); padding: 1.5rem; border-radius: 8px; margin-bottom: 2rem; border: 1px solid var(--border-color); }
        label { display: block; margin-bottom: 0.5rem; color: var(--text-secondary); }
        input, select { width: 100%; padding: 0.5rem; margin-bottom: 1rem; border-radius: 4px; border: 1px solid var(--border-color); background: var(--background-primary); color: var(--text-primary); }
        button { padding: 0.6rem 1.2rem; border: none; border-radius: 4px; background-color: var(--accent-primary); color: #2a2421; cursor: pointer; font-weight: bold; }
        button.delete { background-color: var(--status-error); }
        table { width: 100%; border-collapse: collapse; margin-top: 1rem; }
        th, td { text-align: left; padding: 0.8rem; border-bottom: 1px solid var(--border-color); }
        th { color: var(--text-secondary); }
        td { font-family: monospace; font-size: 0.9rem; word-break: break-all; }
        .logout { float: right; background: none; border: none; color: var(--accent-primary); cursor: pointer; }
...
<div class="container">
    ...
    <div class="card">
        <h2>Add New User</h2>
        <form id="addUserForm">
            <label for="expiration">Expiration (in days from now)</label>
            <input type="number" id="expiration" value="30" required>
            <label for="note">Note (optional)</label>
            <input type="text" id="note" placeholder="e.g., John Doe's account">
...
<script>
    ...
    async function fetchUsers() {
        ...
        try {
            const response = await fetch('/admin/api/users', { headers });
            if (!response.ok) {
            ...
            container.innerHTML = \`<p style="color:var(--status-error)">\${error.message}</p>\`;
        }
    }
    ...
    function renderTable(users) {
        ...
        let tableHTML = '<table><thead><tr><th>UUID</th><th>Status</th><th>Expires</th><th>Note</th><th>Actions</th></tr></thead><tbody>';
        const now = Math.floor(Date.now() / 1000);
        users.forEach(user => {
            const expirationDate = new Date(user.expiration * 1000);
            const isExpired = user.expiration < now;
            const statusClass = user.status === 'active' && !isExpired ? 'active' : 'inactive';
            tableHTML += \`<tr>
                <td class="tooltip" onclick="copyText('\${user.uuid}')">\${user.uuid}<span class="tooltiptext">Click to copy</span></td>
                <td><span class="\${statusClass}">\${isExpired ? 'Expired' : user.status}</span></td>
                <td>\${expirationDate.toLocaleDateString()} \${expirationDate.toLocaleTimeString()}</td>
                <td>\${user.note || '-'}</td>
                <td><button class="delete" onclick="deleteUser('\${user.uuid}')">Delete</button></td>
            </tr>\`;
        });
    ...
    async function deleteUser(uuid) {
        if (!confirm(\`Are you sure you want to delete user \${uuid}?\`)) return;
        try {
            const response = await fetch(\`/admin/api/users/\${uuid}\`, { method: 'DELETE', headers });
    ...
    document.getElementById('addUserForm').addEventListener('submit', async (e) => {
        e.preventDefault();
        const expiration_days = document.getElementById('expiration').value;
        const note = document.getElementById('note').value;
        try {
            const response = await fetch('/admin/api/users', {
                method: 'POST',
                headers,
                body: JSON.stringify({ expiration_days, note })
            });
            if (!response.ok) throw new Error('Failed to add user: ' + await response.text());
            const newUser = await response.json();
            alert(\`User created successfully! UUID: \${newUser.uuid}\`);
            e.target.reset();
            await fetchUsers();
        } catch (error) {
            alert(error.message);
...
// --- USER-FACING PAGE HTML, CSS, JS ---
// These functions are unchanged from your original script.
