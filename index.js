// ------------------------------------------------------------------
// 🚀 VLESS Proxy Worker - Enhanced Stability & Smart Relay 🚀
// ------------------------------------------------------------------
// This version includes refined connection handling and clearer logging
// to help diagnose and resolve communication issues between the
// Cloudflare Worker and the backend PROXYIP server.

import { connect } from 'cloudflare:sockets';

// --- CONFIGURATION ---
const Config = {
  // IMPORTANT: This is a fallback. You MUST set the PROXYIP environment
  // variable in your Cloudflare Worker settings for this to work correctly.
  proxyIPs: ['1.1.1.1:443'],
  scamalytics: {
  ...
  fromEnv(env) {
    // Select the PROXYIP from environment variables. If not set, use the fallback.
  ...
  readableStream.pipeTo(new WritableStream({
    async write(chunk, controller) {
      if (hasAuthenticated && remoteSocket) {
        // If already authenticated, just forward the data to the remote server.
        try {
          await remoteSocket.write(chunk);
        } catch (err) {
          console.error('[RelayError] Error writing to remote socket:', err);
          controller.error(new Error('Failed to write to remote socket.'));
        }
        return;
      }

      // This is the first chunk, which contains the VLESS header for authentication.
      ...
      // Check if the user UUID is valid and active.
      ...
      // <<<<< The key part is here: Establishing the connection to your backend server >>>>>
      try {
        ...
        // Pipe data from the remote server back to the client WebSocket.
        const remoteReadable = socket.readable.pipeTo(new WritableStream({
            write(value) {
            ...
            close() {
                console.log('[RelayClose] Remote socket readable stream closed.');
            },
            abort(err) {
                console.error('[RelayAbort] Remote socket readable stream aborted:', err);
            },
        })).catch(err => {
            console.error('[RelayPipelineFailure] Remote readable pipeline failed:', err.message);
        });

        // Forward the initial authentication chunk to the remote server.
        await remoteSocket.write(chunk);

        // Prevent the worker from terminating while the connection is active.
        ctx.waitUntil(Promise.all([remoteReadable]));

      } catch (err) {
        // <<<<< Smart error logging for debugging connection failures >>>>>
        ...
        console.error("[Hint] Please check three things: 1) Is the PROXYIP environment variable set correctly in Cloudflare dashboard? 2) Is your backend server online and is the VLESS service running? 3) Is the firewall on your backend server allowing connections from Cloudflare IPs?");
        return controller.error(new Error('Relay connection failed. Check Worker logs.'));
      }
    },
    close() {
...
function makeReadableWebSocketStream(webSocket, earlyDataHeader) {
  ...
  return new ReadableStream({
    start(controller) {
      ...
      webSocket.addEventListener('close', () => { if (!readableStreamCancel) { try { controller.close(); } catch(e) {} } });
...
function safeCloseWebSocket(socket, code, reason) {
  try {
    if (socket && socket.readyState !== CONST.WS_READY_STATE_CLOSED) {
...
function generateBeautifulConfigPage(userID, hostName, proxyAddress) { const dream = buildLink({ core: 'xray', proto: 'tls', userID, hostName, address: hostName, port: 443, tag: `${hostName}-Xray` }); const freedom = buildLink({ core: 'sb', proto: 'tls', userID, hostName, address: hostName, port: 443, tag: `${hostName}-Singbox` }); const configs = { dream, freedom }; const subXrayUrl = `https://${hostName}/xray/${userID}`; const subSbUrl = `https://${hostName}/sb/${userID}`; const clientUrls = { clashMeta: `clash://install-config?url=${encodeURIComponent(`https://sub.revil.workers.dev/sub/clash-meta?url=${subSbUrl}&remote_config=&udp=false&ss_uot=false&show_host=false&forced_ws0rtt=true`)}`, hiddify: `hiddify://install-config?url=${encodeURIComponent(subXrayUrl)}`, v2rayng: `v2rayng://install-config?url=${encodeURIComponent(subXrayUrl)}`, exclave: `sn://subscription?url=${encodeURIComponent(subSbUrl)}`, }; return `<!doctype html><html lang="en"><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-w_id_th, initial-scale=1.0" /><title>VLESS Proxy Configuration</title><link rel="icon" href="https://raw.githubusercontent.com/NiREvil/zizifn/main/assets/favicon.png" type="image/png"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Fira+Code:wght@300..700&display=swap" rel="stylesheet"><style>${getPageCSS()}</style></head><body data-proxy-ip="${proxyAddress}">${getPageHTML(configs, clientUrls)}<script>${getPageScript()}</script></body></html>`;}
function getAdminLoginHTML() { return `<!DOCTYPE html><html><head><title>Admin Login</title><meta name="viewport" content="w_id_th=device-w_id_th, initial-scale=1"><style>body{display:flex;justify-content:center;align-items:center;height:100vh;background:#1a1a1a;font-family:sans-serif;margin:0;}div{padding:2rem;background:#2a2a2a;border-radius:8px;color:white;text-align:center;w_id_th:90%;max-w_id_th:400px;}input,button{w_id_th:100%;padding:12px;margin-top:10px;border-radius:5px;border:1px sol_id #444;background:#333;color:white;box-sizing:border-box;font-size:16px;}button{cursor:pointer;background:#007bff;}p{color:red;min-height:1.2em;}</style></head><body><div><h2>Admin Login</h2><input type="password" _id="admin-key" placeholder="Enter Admin Key"><button onclick="login()">Login</button><p _id="error-message"></p></div><script>async function login() { const key = document.getElementBy_id_('admin-key').value; const errorP = document.getElementBy_id_('error-message'); errorP.textContent = ''; if (!key) { errorP.textContent = 'Key cannot be empty.'; return; } try { const response = await fetch('/admin/api/users', { headers: { 'Authorization': key } }); if (response.ok) { localStorage.setItem('admin_key', key); w_id_ndow.location.href = '/admin/dashboard'; } else if (response.status === 401) { errorP.textContent = 'Inval_id Key. Access Denied.'; } else { errorP.textContent = 'An unknown error occurred.'; } } catch (err) { errorP.textContent = 'Failed to connect to the server.'; } } document.getElementBy_id_('admin-key').addEventListener('keyup', (event) => { if (event.key === 'Enter') login(); });</script></body></html>`; }
function getAdminDashboardHTML() { return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="w_id_th=device-w_id_th, initial-scale=1.0"><title>Admin Dashboard</title><style>body{background:#1a1a1a;font-family:sans-serif;color:#fff;padding:10px;}.dashboard{max-w_id_th:1000px;margin:auto;background:#2a2a2a;border-radius:15px;padding:20px;}h1,h2{text-align:center;}.create-section{background:#333;padding:15px;border-radius:10px;margin-bottom:20px;display:flex;flex-wrap:wrap;gap:10px;}input,button{padding:10px;border:none;border-radius:5px;background:#444;color:#fff;font-size:14px;}button{background:#007bff;cursor:pointer;}#new-_id_{flex:1 1 200px;}.user-list{overflow-x:auto;}table{w_id_th:100%;border-collapse:collapse;}th,td{padding:12px;text-align:left;border-bottom:1px sol_id #444;word-break:break-all;}.expired{color:#ff5252;}.active{color:#4caf50;}.actions-cell button{background:#e53935;}</style></head><body><div class="dashboard"><h1>Admin Dashboard</h1><div class="create-section"><input type="text" _id="new-_id_" placeholder="User ID (UU_id_)" readonly><button onclick="generateUU_id_()">Generate</button><input type="date" _id="exp-date"><input type="time" _id="exp-time"><input type="text" _id="notes" placeholder="Notes (optional)"><button onclick="createUser()">Create User</button></div><div class="user-list"><h2>User List</h2><table _id="user-table"><thead><tr><th>ID</th><th>Expires</th><th>Created</th><th>Last Access</th><th>Status</th><th>Notes</th><th>Actions</th></tr></thead><tbody><tr><td colspan="7" style="text-align:center;">Loading...</td></tr></tbody></table></div></div><script>const adminKey = localStorage.getItem('admin_key'); if (!adminKey) w_id_ndow.location.href = '/admin/login'; const apiHeaders = { 'Content-Type': 'application/json', 'Authorization': adminKey }; async function apiFetch(endpoint, options={}){const res = await fetch('/admin/api' + endpoint, {...options, headers: apiHeaders}); if(res.status === 401){alert('Unauthorized!');w_id_ndow.location.href='/admin/login'; throw new Error('Unauthorized');} return res;} function generateUU_id_(){document.getElementBy_id_('new-_id_').value = crypto.randomUU_id_();} function setupDefaultDateTime() { const now = new Date(); now.setMonth(now.getMonth() + 1); document.getElementBy_id_('exp-date').value = now.toISOString().slice(0,10); document.getElementBy_id_('exp-time').value = now.toTimeString().slice(0,5); } async function createUser(){const _id_=document.getElementBy_id_('new-_id_').value;const date=document.getElementBy_id_('exp-date').value;const time=document.getElementBy_id_('exp-time').value;const notes=document.getElementBy_id_('notes').value;if(!_id_||!date||!time)return alert('Please fill all required fields.');try {const res=await apiFetch('/users',{method:'POST',body:JSON.stringify({_id_,expiration_date:date,expiration_time:time,notes})});const data=await res.json();if(res.ok){alert(data.message);loadUsers();generateUU_id_();document.getElementBy_id_('notes').value='';}else{throw new Error(data.error||'Failed to create user');}}catch(e){alert('Error: '+e.message);}} async function loadUsers(){const tbody=document.getElementBy_id_('user-table').querySelector('tbody');tbody.innerHTML='<tr><td colspan="7" style="text-align:center;">Loading...</td></tr>';try{const res=await apiFetch('/users');const users=await res.json();tbody.innerHTML='';const now=Date.now()/1000;users.forEach(u=>{const isExpired=u.expiration_timestamp<=now;const statusText=isExpired?'Expired':u.status;const statusClass=isExpired?'expired':'active';const expiry=new Date(u.expiration_timestamp*1000).toLocaleString();const created=new Date(u.created_at*1000).toLocaleDateString();const lastAccess=u.last_accessed>0?new Date(u.last_accessed*1000).toLocaleString():'Never';tbody.innerHTML+=\`<tr><td>\${u._id_}</td><td class="\${statusClass}">\${expiry}</td><td>\${created}</td><td>\${lastAccess}</td><td class="\${statusClass}">\${statusText}</td><td>\${u.notes||''}</td><td class="actions-cell"><button onclick="deleteUser('\${u._id_}')">Delete</button></td></tr>\`;});}catch(e){tbody.innerHTML=\`<tr><td colspan="7" style="text-align:center;color:red;">Failed to load users: \${e.message}</td></tr>\`;}} async function deleteUser(_id_){if(!confirm(\`Are you sure you want to delete user \${_id_}?\`))return;try{const res=await apiFetch(\`/users/\${_id_}\`,{method:'DELETE'});const data=await res.json();if(res.ok){alert(data.message);loadUsers();}else{throw new Error(data.error||'Failed to delete user');}}catch(e){alert('Error: '+e.message);}} w_id_ndow.onload=()=>{generateUU_id_();setupDefaultDateTime();loadUsers();};</script></body></html>`; }
function getPageCSS() { return `* { margin: 0; padding: 0; box-sizing: border-box; } @font-face { font-family: "Aldine 401 BT Web"; src: url("https://pub-7a3b428c76aa411181a0f4dd7fa9064b.r2.dev/Aldine401_Mersedeh.woff2") format("woff2"); font-weight: 400; font-style: normal; font-display: swap; } @font-face { font-family: "Styrene B LC"; src: url("https://pub-7a3b428c76aa411181a0f4dd7fa9064b.r2.dev/StyreneBLC-Regular.woff2") format("woff2"); font-weight: 400; font-style: normal; font-display: swap; } @font-face { font-family: "Styrene B LC"; src: url("https://pub-7a3b428c76aa411181a0f4dd7fa9064b.r2.dev/StyreneBLC-Medium.woff2") format("woff2"); font-weight: 500; font-style: normal; font-display: swap; } :root { --background-primary: #2a2421; --background-secondary: #35302c; --background-tertiary: #413b35; --border-color: #5a4f45; --border-color-hover: #766a5f; --text-primary: #e5dfd6; --text-secondary: #b3a89d; --text-accent: #ffffff; --accent-primary: #be9b7b; --accent-secondary: #d4b595; --accent-tertiary: #8d6e5c; --accent-primary-darker: #8a6f56; --button-text-primary: #2a2421; --button-text-secondary: var(--text-primary); --shadow-color: rgba(0, 0, 0, 0.35); --shadow-color-accent: rgba(190, 155, 123, 0.4); --border-radius: 8px; --transition-speed: 0.2s; --transition-speed-fast: 0.1s; --transition-speed-medium: 0.3s; --transition-speed-long: 0.6s; --status-success: #70b570; --status-error: #e05d44; --status-warning: #e0bc44; --status-info: #4f90c4; --serif: "Aldine 401 BT Web", "Times New Roman", Times, Georgia, ui-serif, serif; --sans-serif: "Styrene B LC", -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, "Noto Color Emoji", sans-serif; --mono-serif: "Fira Code", Cantarell, "Courier Prime", monospace; } body { font-family: var(--sans-serif); font-size: 16px; background-color: var(--background-primary); color: var(--text-primary); padding: 3rem; line-height: 1.5; -webkit-font-smoothing: antialiased; } .container { max-w_id_th: 800px; margin: 20px auto; padding: 0 12px; border-radius: var(--border-radius); box-shadow: 0 6px 15px rgba(0,0,0,0.2), 0 0 25px 8px var(--shadow-color-accent); transition: box-shadow var(--transition-speed-medium) ease; } .container:hover { box-shadow: 0 8px 20px rgba(0,0,0,0.25), 0 0 35px 10px var(--shadow-color-accent); } .header { text-align: center; margin-bottom: 40px; padding-top: 30px; } .header h1 { font-family: var(--serif); font-weight: 400; font-size: 1.8rem; color: var(--text-accent); margin-bottom: 2px; } .header p { color: var(--text-secondary); font-size: 0.6rem; } .config-card { background: var(--background-secondary); border-radius: var(--border-radius); padding: 20px; margin-bottom: 24px; border: 1px sol_id var(--border-color); transition: border-color var(--transition-speed) ease, box-shadow var(--transition-speed) ease; } .config-card:hover { border-color: var(--border-color-hover); box-shadow: 0 4px 8px var(--shadow-color); } .config-title { font-family: var(--serif); font-size: 1.6rem; color: var(--accent-secondary); margin-bottom: 16px; padding-bottom: 13px; border-bottom: 1px sol_id var(--border-color); display: flex; align-items: center; justify-content: space-between; } .refresh-btn { position: relative; overflow: h_id_den; display: flex; align-items: center; gap: 4px; font-family: var(--serif); font-size: 12px; padding: 6px 12px; border-radius: 6px; color: var(--accent-secondary); background-color: var(--background-tertiary); border: 1px sol_id var(--border-color); cursor: pointer; transition: all var(--transition-speed) ease; } .refresh-btn::before { content: ''; position: absolute; top: 0; left: 0; w_id_th: 100%; height: 100%; background: linear-gradient(120deg, transparent, rgba(255,255,255,0.2), transparent); transform: translateX(-100%); transition: transform var(--transition-speed-long) ease; } .refresh-btn:hover { letter-spacing: 0.5px; font-weight: 600; background-color: #4d453e; color: var(--accent-primary); border-color: var(--border-color-hover); transform: translateY(-2px); box-shadow: 0 4px 8px var(--shadow-color); } .refresh-btn:hover::before { transform: translateX(100%); } .refresh-btn:active { transform: translateY(0px) scale(0.98); box-shadow: none; } .refresh-icon { w_id_th: 12px; height: 12px; stroke: currentColor; } .config-content { background: var(--background-tertiary); border-radius: var(--border-radius); padding: 16px; margin-bottom: 20px; border: 1px sol_id var(--border-color); } .config-content pre { overflow-x: auto; font-family: var(--mono-serif); font-size: 12px; color: var(--text-primary); margin: 0; white-space: pre-wrap; word-break: break-all; } .button { display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 8px 16px; border-radius: var(--border-radius); font-size: 15px; font-weight: 500; cursor: pointer; border: 1px sol_id var(--border-color); background-color: var(--background-tertiary); color: var(--button-text-secondary); text-decoration: none; transition: all var(--transition-speed) ease; } .copy-buttons { position: relative; display: flex; gap: 4px; overflow: h_id_den; align-self: center; font-family: var(--serif); font-size: 13px; padding: 6px 12px; border-radius: 6px; color: var(--accent-secondary); border: 1px sol_id var(--border-color); } .copy-buttons::before, .client-btn::before { content: ''; position: absolute; top: 0; left: 0; w_id_th: 100%; height: 100%; background: linear-gradient(120deg, transparent, rgba(255,255,255,0.2), transparent); transform: translateX(-100%); transition: transform var(--transition-speed-long) ease; z-index: 0; } .copy-buttons:hover::before, .client-btn:hover::before { transform: translateX(100%); } .copy-buttons:hover { background-color: #4d453e; letter-spacing: 0.5px; font-weight: 600; border-color: var(--border-color-hover); transform: translateY(-2px); box-shadow: 0 4px 8px var(--shadow-color); } .copy-buttons:active { transform: translateY(0px) scale(0.98); box-shadow: none; } .copy-icon { w_id_th: 12px; height: 12px; stroke: currentColor; position: relative; z-index: 1; } .client-buttons { display: gr_id; gr_id-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 12px; margin-top: 16px; } .client-btn { w_id_th: 100%; background-color: var(--accent-primary); color: var(--background-tertiary); border-color: var(--accent-primary-darker); position: relative; overflow: h_id_den; box-shadow: 0 2px 5px rgba(0,0,0,0.15); } .client-btn:hover { transform: translateY(-3px); background-color: var(--accent-secondary); color: var(--button-text-primary); box-shadow: 0 5px 15px rgba(190, 155, 123, 0.5); border-color: var(--accent-secondary); } .client-btn:active { transform: translateY(0) scale(0.98); box-shadow: 0 2px 3px rgba(0,0,0,0.2); } .client-icon { w_id_th: 18px; height: 18px; border-radius: 6px; background-color: var(--background-secondary); display: flex; align-items: center; justify-content: center; flex-shrink: 0; } .client-icon svg { w_id_th: 14px; height: 14px; fill: var(--accent-secondary); } .button.copied { background-color: var(--status-success) !important; color: var(--background-tertiary) !important; border-color: var(--status-success) !important; } .footer { text-align: center; margin-top: 20px; padding-bottom: 40px; color: var(--text-secondary); font-size: 10px; } .ip-info-gr_id_ { display: gr_id; gr_id-template-columns: repeat(auto-fit, minmax(230px, 1fr)); gap: 24px; } .ip-info-section { background-color: var(--background-tertiary); border-radius: var(--border-radius); padding: 16px; border: 1px sol_id var(--border-color); display: flex; flex-direction: column; gap: 20px; } .ip-info-header { display: flex; align-items: center; gap: 10px; border-bottom: 1px sol_id var(--border-color); padding-bottom: 10px; } .ip-info-header svg { w_id_th: 20px; height: 20px; stroke: var(--accent-secondary); } .ip-info-header h3 { font-family: var(--serif); font-size: 18px; font-weight: 400; color: var(--accent-secondary); margin: 0; } .ip-info-content { display: flex; flex-direction: column; gap: 10px; } .ip-info-item { display: flex; flex-direction: column; gap: 2px; } .ip-info-item .label { font-size: 11px; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.5px; } .ip-info-item .value { font-size: 14px; color: var(--text-primary); word-break: break-all; line-height: 1.4; min-height: 20px; } .badge { display: inline-flex; align-items: center; justify-content: center; padding: 3px 8px; border-radius: 12px; font-size: 11px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px; } .badge-yes { background-color: rgba(112, 181, 112, 0.15); color: var(--status-success); border: 1px sol_id rgba(112, 181, 112, 0.3); } .badge-no { background-color: rgba(224, 93, 68, 0.15); color: var(--status-error); border: 1px sol_id rgba(224, 93, 68, 0.3); } .badge-neutral { background-color: rgba(79, 144, 196, 0.15); color: var(--status-info); border: 1px sol_id rgba(79, 144, 196, 0.3); } .badge-warning { background-color: rgba(224, 188, 68, 0.15); color: var(--status-warning); border: 1px sol_id rgba(224, 188, 68, 0.3); } .skeleton { display: block; background: linear-gradient(90deg, var(--background-tertiary) 25%, var(--background-secondary) 50%, var(--background-tertiary) 75%); background-size: 200% 100%; animation: loading 1.5s infinite; border-radius: 4px; height: 16px; } @keyframes loading { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } } .country-flag { display: inline-block; w_id_th: 18px; height: auto; max-height: 14px; margin-right: 6px; vertical-align: m_id_dle; border-radius: 2px; } @media (max-w_id_th: 768px) { body { padding: 20px; } .container { padding: 0 14px; } } @media (max-w_id_th: 480px) { body { padding: 16px; } .config-content pre {font-size: 10px;} .header h1 {font-size: 1.6rem;} .client-buttons { gr_id-template-columns: 1fr; } }`; }
function getPageHTML(configs, clientUrls) { return `<div class="container"><div class="header"><h1>VLESS Proxy Configuration</h1><p>Copy the configuration or import directly into your client</p></div><div class="config-card"><div class="config-title"><span>Network Information</span><button _id="refresh-ip-info" class="refresh-btn" aria-label="Refresh IP information"><svg class="refresh-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-w_id_th="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.5 2v6h-6M2.5 22v-6h6M2 11.5a10 10 0 0 1 18.8-4.3M22 12.5a10 10 0 0 1-18.8 4.2" /></svg>Refresh</button></div><div class="ip-info-gr_id_"><div class="ip-info-section"><div class="ip-info-header"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-w_id_th="2" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="2" w_id_th="14" height="20" rx="2" ry="2"></rect><line x1="12" y1="18" x2="12.01" y2="18"></line></svg><h3>Proxy Server</h3></div><div class="ip-info-content"><div class="ip-info-item"><span class="label">Proxy Host</span><span class="value" _id="proxy-host"><span class="skeleton" style="w_id_th: 150px"></span></span></div><div class="ip-info-item"><span class="label">IP Address</span><span class="value" _id="proxy-ip"><span class="skeleton" style="w_id_th: 120px"></span></span></div><div class="ip-info-item"><span class="label">Location</span><span class="value" _id="proxy-location"><span class="skeleton" style="w_id_th: 100px"></span></span></div><div class="ip-info-item"><span class="label">ISP Provider</span><span class="value" _id="proxy-isp"><span class="skeleton" style="w_id_th: 140px"></span></span></div></div></div><div class="ip-info-section"><div class="ip-info-header"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-w_id_th="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 16V7a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v9m16 0H4m16 0 1.28 2.55a1 1 0 0 1-.9 1.45H3.62a1 1 0 0 1-.9-1.45L4 16" /></svg><h3>Your Connection</h3></div><div class="ip-info-content"><div class="ip-info-item"><span class="label">Your IP</span><span class="value" _id="client-ip"><span class="skeleton" style="w_id_th: 110px"></span></span></div><div class="ip-info-item"><span class="label">Location</span><span class="value" _id="client-location"><span class="skeleton" style="w_id_th: 90px"></span></span></div><div class="ip-info-item"><span class="label">ISP Provider</span><span class="value" _id="client-isp"><span class="skeleton" style="w_id_th: 130px"></span></span></div><div class="ip-info-item"><span class="label">Risk Score</span><span class="value" _id="client-proxy"><span class="skeleton" style="w_id_th: 100px"></span></span></div></div></div></div></div><div class="config-card"><div class="config-title"><span>Xray Core Clients</span><button class="button copy-buttons" onclick="copyToClipboard(this, '${configs.dream}')"><svg class="copy-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-w_id_th="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" w_id_th="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>Copy</button></div><div class="config-content"><pre _id="xray-config">${configs.dream}</pre></div><div class="client-buttons"><a href="${clientUrls.h_id_dify}" class="button client-btn"><span class="client-icon"><svg viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" /></svg></span>Import to H_id_dify</a><a href="${clientUrls.v2rayng}" class="button client-btn"><span class="client-icon"><svg viewBox="0 0 24 24"><path d="M12 2L4 5v6c0 5.5 3.5 10.7 8 12.3 4.5-1.6 8-6.8 8-12.3V5l-8-3z" /></svg></span>Import to V2rayNG</a></div></div><div class="config-card"><div class="config-title"><span>Sing-Box Core Clients</span><button class="button copy-buttons" onclick="copyToClipboard(this, '${configs.freedom}')"><svg class="copy-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-w_id_th="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" w_id_th="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>Copy</button></div><div class="config-content"><pre _id="singbox-config">${configs.freedom}</pre></div><div class="client-buttons"><a href="${clientUrls.clashMeta}" class="button client-btn"><span class="client-icon"><svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z" /></svg></span>Import to Clash Meta</a><a href="${clientUrls.exclave}" class="button client-btn"><span class="client-icon"><svg viewBox="0 0 24 24"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zM9 6c0-1.66 1.34-3 3-3s3 1.34 3 3v2H9V6z" /></svg></span>Import to Exclave</a></div></div><div class="footer"><p>© ${new Date().getFullYear()} REvil - All Rights Reserved</p></div></div>`; }
function getPageScript() { return `function copyToClipboard(button, text) { const originalHTML = button.innerHTML; navigator.clipboard.writeText(text).then(() => { button.innerHTML = \`<svg class="copy-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-w_id_th="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg> Copied!\`; button.classList.add("copied"); button.disabled = true; setTimeout(() => { button.innerHTML = originalHTML; button.classList.remove("copied"); button.disabled = false; }, 1200); }).catch(err => { console.error("Failed to copy text: ", err); }); } function updateDisplay(data) { const na = 'N/A'; const proxy = data.proxy || {}; document.getElementBy_id_('proxy-host').textContent = proxy.host || na; document.getElementBy_id_('proxy-ip').textContent = proxy.ip || na; document.getElementBy_id_('proxy-isp').textContent = proxy.isp || na; const p_loc = [proxy.city, proxy.country].filter(Boolean).join(', '); const p_flag = proxy.country ? \`<img src="https://flagcdn.com/w20/\${proxy.country.toLowerCase()}.png" class="country-flag" alt="\${proxy.country}"> \` : ''; document.getElementBy_id_('proxy-location').innerHTML = (p_loc) ? \`\${p_flag}\${p_loc}\` : na; const client = data.client || {}; document.getElementBy_id_('client-ip').textContent = client.ip || na; document.getElementBy_id_('client-isp').textContent = client.isp || na; const c_loc = [client.city, client.country].filter(Boolean).join(', '); const c_flag = client.country ? \`<img src="https://flagcdn.com/w20/\${client.country.toLowerCase()}.png" class="country-flag" alt="\${client.country}"> \` : ''; document.getElementBy_id_('client-location').innerHTML = (c_loc) ? \`\${c_flag}\${c_loc}\` : na; const risk = client.risk; let riskText = "Unknown"; let badgeClass = "badge-neutral"; if (risk && typeof risk.score !== 'undefined') { const riskLevel = risk.risk.charAt(0).toUpperCase() + risk.risk.slice(1); riskText = \`\${risk.score} - \${riskLevel}\`; switch (risk.risk.toLowerCase()) { case "low": badgeClass = "badge-yes"; break; case "medium": badgeClass = "badge-warning"; break; case "high": case "very high": badgeClass = "badge-no"; break; } } document.getElementBy_id_('client-proxy').innerHTML = \`<span class="badge \${badgeClass}">\${riskText}</span>\`; } async function loadNetworkInfo() { try { const response = await fetch('/api/network-info'); if (!response.ok) throw new Error(\`API request failed with status \${response.status}\`); const data = await response.json(); updateDisplay(data); } catch (error) { console.error('Failed to load network info:', error); const errorData = { client: {}, proxy: { host: document.body.getAttribute('data-proxy-ip') || 'N/A' } }; updateDisplay(errorData); } } document.getElementBy_id_('refresh-ip-info')?.addEventListener('click', function() { const button = this; const icon = button.querySelector('.refresh-icon'); button.disabled = true; if (icon) icon.style.animation = 'spin 1s linear infinite'; const resetToSkeleton = () => { document.querySelectorAll('.value').forEach(el => { el.innerHTML = \`<span class="skeleton" style="w_id_th: \${Math.floor(80 + Math.random() * 60)}px"></span>\`; }); }; resetToSkeleton(); loadNetworkInfo().finally(() => setTimeout(() => { button.disabled = false; if (icon) icon.style.animation = ''; }, 500)); }); const style = document.createElement('style'); style.textContent = \`@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }\`; document.head.appendChild(style); document.addEventListener('DOMContentLoaded', loadNetworkInfo);`; }
