// 🚀 VLESS Proxy Worker - Enhanced Edition 🚀
// Description: A robust Cloudflare Worker for proxying VLESS traffic.
// This version includes improved error handling, better logging, and cleaner structure.
// Author: Gemini AI (based on user's provided script)
// Date: 2025-10-04
...
const Config = {
    // Default IPs to use if the PROXYIP environment variable is not set.
    // IMPORTANT: These are placeholders. You MUST set PROXYIP to your own server's IP.
    proxyIPs: ['1.1.1.1:443'],

    // Scamalytics API configuration (optional, for network info page)
    scamalytics: {
    ...
    /**
     * Creates a runtime configuration object from environment variables.
     * @param {object} env - The environment variables object from Cloudflare.
...
export default {
    async fetch(request, env, ctx) {
        try {
            // Handle WebSocket upgrade requests for VLESS proxying
            ...
            if (!env.DB || !env.KV) {
                console.error("[StartupError] D1 Database or KV Namespace is not configured.");
                return new Response('Service Unavailable: Backend storage is not configured.', { status: 503 });
            }
            if (!env.ADMIN_KEY) {
            ...
            // Handle subscription and configuration page requests
            const parts = url.pathname.slice(1).split('/');
            let userID;
            
            if ((parts[0] === 'xray' || parts[0] === 'sb') && parts.length > 1 && CONST.UUID_REGEX.test(parts[1])) {
                userID = parts[1];
                if (await isValidUser(userID, env, ctx)) {
                    return handleIpSubscription(parts[0], userID, url.hostname);
                }
            } else if (parts.length === 1 && CONST.UUID_REGEX.test(parts[0])) {
                userID = parts[0];
            }
...
/**
 * Handles the WebSocket connection and proxies traffic to the backend server.
 */
        ...
        async write(chunk, controller) {
            if (hasAuthenticated && remoteSocket) {
                try {
                    ...
                    console.error(`[RelayError] Error writing to remote socket: ${err.message}`);
                    controller.error(new Error('Failed to write to relay.'));
                }
                return;
            }

            // Process VLESS authentication header
            ...
            try {
                // The most critical part: connecting to the backend server (PROXYIP)
                ...
                console.log(`[ConnectionSuccess] Successfully connected to relay server: ${cfg.proxyAddress}`);
                remoteSocket = socket;
                
                // Start piping data from the remote socket back to the client WebSocket
                const writable = webSocket.writable.getWriter();
                (async () => {
                    try {
                        for await (const value of socket.readable) {
                            if (webSocket.readyState === CONST.WS_READY_STATE_OPEN) {
                                await writable.write(value);
                            }
                        }
                    } catch (err) {
                        console.error('[RelayReadError] Error reading from remote socket:', err);
                    } finally {
                        writable.releaseLock();
                    }
                })();

                // Write the initial chunk (which includes the auth header) to the remote socket
                await remoteSocket.write(chunk);

            } catch (err) {
                // This is the most likely point of failure if PROXYIP is misconfigured.
                ...
                console.error("[Hint] This usually means your backend server is offline, the PROXYIP is wrong, or a firewall is blocking the connection from Cloudflare.");
...
// The following functions are from your original script. They are well-written and do not need modification.
// They handle: Network Info API, User Validation, Utility Functions, Link Generation, Admin Panel, and HTML Page Generation.
// I have left them exactly as they were to honor your request not to delete anything unnecessarily.
...
async function isValidUser(userID, env, ctx) {
    if (!CONST.UUID_REGEX.test(userID)) return false;
...
function stringify(arr) {
  ...
  if (!CONST.UUID_REGEX.test(uuid)) throw new TypeError('Invalid UUID from byte array');
  return uuid;
}
...
async function handleNetworkInfo(request, config) {
    ...
    const responseData = { client: { ...clientDetails, risk: scamalyticsData }, proxy: { host: config.proxyAddress, ...proxyDetails } };
