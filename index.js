...
async function getAndCacheUser(uuid, db, cache) {
	if (!db || !cache) {
		const envUUIDs = (config.userID).split(',');
...
function handleDohProxy(request, dohURL) { return fetch(dohURL, request); }
...
// ##################################################################
// # NEW, IMPROVED AND STABLE SUBSCRIPTION GENERATOR
// ##################################################################
