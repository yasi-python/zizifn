    ...
    try {
      ...
      if (!upgradeHeader || upgradeHeader !== "websocket") {
        switch (url.pathname) {
          ...
          case "/scamalytics-lookup": { // New endpoint for Scamalytics
      ...
      new WritableStream({
        async write(chunk, controller) {
          ...
          const {
            ...
            rawDataIndex, streamVersion = new Uint8Array([0, 0]), isUDP,
          ...
          handleTCPOutBound(
            remoteSocketWapper, addressRemote, portRemote, rawClientData,
            webSocket, streamResponseHeader, log
          );
        },
...
function processStreamHeader(chunk, userCode) {
  ...
  return {
    ...
    rawDataIndex: addressValueIndex + addressLength, streamVersion: version, isUDP,
  };
}
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, streamResponseHeader, log) {
  ...
  async function retry() {
    ...
    tcpSocket.closed.catch(e => console.log("retry tcpSocket closed error", e)).finally(() => safeCloseWebSocket(webSocket));
...
async function remoteSocketToWS(remoteSocket, webSocket, streamResponseHeader, retry, log) {
    ...
    new WritableStream({
      async write(chunk, controller) {
        ...
        if (webSocket.readyState !== WS_READY_STATE_OPEN) {
          controller.error("webSocket is not open"); return;
        }
        if (vlessHeader) {
  ...
  ).catch(e => { console.error("remoteSocketToWS has error", e.stack || e); safeCloseWebSocket(webSocket); });
  if (hasIncomingData === false && retry) { log("retry connection"); retry(); }
}
...
async function getDianaConfig(currentUuid, hostName) {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch HTML: ${response.status} ${response.statusText} (URL: ${HTML_URL})`);
    }
    let html = await response.text();
    html = html
      .replace(/<body(.*?)>/i, `<body$1 data-proxy-ip="${proxyIP}">`)
      .replace(/{{PROXY_IP}}/g, proxyIP)
      .replace(/{{LAST_UPDATED}}/g, new Date().toLocaleString("en-US", { hour: "numeric", minute: "numeric", hour12: false, day: "numeric", month: "long" }))
