...
// v2.0 - With Automatic TLS Detection & Enhanced Logging
// -----------------------------------------------------
...
// --- CONFIGURATION ---
const Config = {
  proxyIPs: ['1.1.1.1:443'], // Fallback if PROXYIP is not set
  // Standard ports for secure (TLS) connections
  tlsPorts: [443, 8443, 2053, 2083, 2087, 2096],
  scamalytics: {
  ...
  fromEnv(env) {
    ...
    return {
      ...
      proxyIP: proxyHost,
      proxyPort: port,
      // Automatically detect the need for TLS based on the port
      useTLS: this.tlsPorts.includes(port),
      scamalytics: {
...
async function handleWebSocket(request, env, ctx) {
  ...
  readableStream.pipeTo(new WritableStream({
    async write(chunk, controller) {
      ...
      try {
        // <<<<< This is the key, intelligent, and modified section >>>>>
        const connectionType = cfg.useTLS ? 'TLS (Secure)' : 'TCP (Plain)';
        console.log(`[ConnectionAttempt] Attempting a ${connectionType} connection to relay server: ${cfg.proxyAddress}`);

        // Establishes a secure or plain connection intelligently based on the port
        const socketOptions = cfg.useTLS ? { secureTransport: 'On' } : {};
        const socket = await connect({ hostname: cfg.proxyIP, port: cfg.proxyPort }, socketOptions);

        console.log(`[ConnectionSuccess] Successfully connected to relay server via ${connectionType}.`);
        remoteSocket = socket;
        ...
        // <<<<< Smarter and more precise error logging for debugging >>>>>
        const connectionType = cfg.useTLS ? 'TLS (Secure)' : 'TCP (Plain)';
        console.error(`[ConnectionFailure] CRITICAL: Failed to establish a ${connectionType} connection to relay server at ${cfg.proxyAddress}.`);
        console.error(`[ConnectionFailure] Error details: ${err.message}`);
        console.error("[Hint] Please check three things: 1) Is the PROXYIP environment variable set correctly in Cloudflare dashboard? 2) Is your backend server online and is the VLESS service running? 3) Is the firewall on your backend server allowing connections from Cloudflare IPs on the correct port?");
