...
// -----------------------------------------------------
// This script acts as a secure relay for VLESS traffic,
// includes a full-featured user management system with D1 and KV,
// and features a server-side rendered network info panel for optimal performance.
//
...
// --- CONFIGURATION ---
const Config = {
  // Fallback relay server if the PROXYIP environment variable is not set.
  // This should be the IP:PORT of your backend VLESS server.
  proxyIPs: ['1.1.1.1:443'],

  // Scamalytics configuration for client risk assessment.
  scamalytics: {
   ...
   * @param {object} env - The environment variables from Cloudflare.
...
export default {
   ...
   * @param {object} env - Cloudflare Environment Bindings (D1, KV, Secrets)
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      // A smart, server-side API endpoint for all network information
      if (url.pathname === '/api/network-info') {
        return handleNetworkInfo(request, cfg);
      }

      // Ensure D1 and KV are bound for user management and admin panel
      if (!env.DB || !env.KV) {
        return new Response('Service Unavailable: D1 Database or KV Namespace is not configured.', { status: 503 });
      }
      if (!env.ADMIN_KEY) {
        // Log an error but don't expose it to the user for security
      ...
      // Routing for user config pages and subscriptions
      const parts = url.pathname.slice(1).split('/');
      let userID;
      // Subscription link format: /xray/{UUID} or /sb/{UUID}
      ...
      // Config page format: /{UUID}
      } else if (parts.length === 1 && isValidUUID(parts[0])) {
        userID = parts[0];
      }
      
      // If a valid user is found, show their config page
    ...
    const getIpDetails = async (ip) => {
        ...
        try {
            ...
            return {
                ip: data.ip,
                city: data.city,
                country: data.country, // ipinfo provides country code (e.g., US)
                isp: data.org,
            };
        } catch (error) {
  ...
  readableStream.pipeTo(new WritableStream({
    async write(chunk, controller) {
      if (hasAuthenticated && remoteSocket) {
        // After authentication, forward all data directly to the relay server
        await remoteSocket.write(chunk);
        return;
      }

      // --- This block runs only for the first chunk of data ---
      const { hasError, message, uuid } = await processVlessHeader(chunk);
      if (hasError) {
        console.error(`VLESS Header Error: ${message}`);
        return controller.error(new Error(message));
      }

      // Authenticate the user
      if (!await isValidUser(uuid, env, ctx)) {
        console.warn(`Authentication failed for UUID: ${uuid}`);
        return controller.error(new Error('Invalid user'));
      }
      
      console.log(`User ${uuid} authenticated successfully.`);
      hasAuthenticated = true;

      try {
        // Connect to the backend relay server (PROXYIP)
        const socket = await connect({ hostname: cfg.proxyIP, port: cfg.proxyPort });
        remoteSocket = socket;
        
        // Pipe data from the relay server back to the client WebSocket
        const writer = socket.writable.getWriter();
        const reader = socket.readable.getReader();

        // Forward the initial authentication chunk to the relay server
        await writer.write(chunk);
        writer.releaseLock();
        
        // Asynchronously read from remote and write to WebSocket
        (async () => {
          while (true) {
            try {
              ...
              console.error('Error reading from remote socket:', err);
              break;
            }
          }
        })();
        ...
        console.error(`Failed to connect to relay server ${cfg.proxyAddress}:`, err);
    ...
    console.error('WebSocket pipeline failed:', err);
    safeCloseSocket(remoteSocket);
...
/**
 * Checks if a user is valid by querying D1, with KV caching.
 * @param {string} userID - The UUID of the user to validate.
 * @param {object} env - The Cloudflare environment bindings.
 * @param {object} ctx - The execution context.
 * @returns {Promise<boolean>} True if the user is valid and active.
 */
async function isValidUser(userID, env, ctx) {
    if (!isValidUUID(userID)) return false;
    
    // Use KV as a fast cache for user validity
    ...
    try {
        ...
        // Check if user exists, is active, and not expired
        if (user && user.status === 'active' && user.expiration_timestamp > now) {
            // Update last access time without blocking the request
            ctx.waitUntil(
              env.DB.prepare('UPDATE users SET last_accessed = ? WHERE id = ?').bind(now, userID).run()
            );
            // Cache the valid status in KV until the user's expiration time
            await env.KV.put(cacheKey, 'true', { expiration: user.expiration_timestamp });
            return true;
        } else {
            // Cache invalid status for 1 hour to reduce DB load for invalid users
            await env.KV.put(cacheKey, 'false', { expirationTtl: 3600 });
            return false;
        }
    } catch (e) {
        console.error('D1 database query failed in isValidUser:', e);
        // In case of DB error, deny access to be safe
        return false;
    }
}
...
function buildLink({ core, proto, userID, hostName, address, port, tag }) {
  ...
  return createVlessLink({
    userID, address, port, host: hostName, path: p.path(),
    security: p.security, sni: p.security === 'tls' ? hostName : undefined,
    fp: p.fp, alpn: p.alpn, extra: p.extra, name,
  });
}
...
async function handleIpSubscription(core, userID, hostName) {
  ...
  } catch (e) { console.error('Failed to fetch IP list for subscription:', e); }
...
async function handleAdminRoutes(request, env) {
  ...
  // Serve static HTML for login and dashboard pages
  ...
  try {
    ...
    if (request.method === 'POST' && path === '/api/users') {
      ...
      await env.DB.prepare(
        'INSERT INTO users (id, expiration_timestamp, created_at, last_accessed, status, notes) VALUES (?, ?, ?, ?, ?, ?)'
      ).bind(id, expirationTimestamp, now, 0, 'active', notes || null).run();

      await env.KV.delete(`user-validity:${id}`); // Invalidate cache
...
function generateBeautifulConfigPage(userID, hostName, proxyAddress) {
  ...
  <head>
    ...
    <link rel="icon" href="https://raw.githubusercontent.com/NiREvil/zizifn/main/assets/favicon.png" type="image/png">
...
function getAdminLoginHTML() {
  return `<!DOCTYPE html><html><head><title>Admin Login</title><meta name="viewport" content="width=device-width, initial-scale=1"><style>body{display:flex;justify-content:center;align-items:center;height:100vh;background:#1a1a1a;font-family:sans-serif;margin:0;}div{padding:2rem;background:#2a2a2a;border-radius:8px;color:white;text-align:center;width:90%;max-width:400px;}input,button{width:100%;padding:12px;margin-top:10px;border-radius:5px;border:1px solid #444;background:#333;color:white;box-sizing:border-box;font-size:16px;}button{cursor:pointer;background:#007bff;}p{color:red;min-height:1.2em;}</style></head><body><div><h2>Admin Login</h2><input type="password" id="admin-key" placeholder="Enter Admin Key"><button onclick="login()">Login</button><p id="error-message"></p></div><script>
    async function login() {
        const key = document.getElementById('admin-key').value;
        const errorP = document.getElementById('error-message');
        errorP.textContent = '';
        if (!key) { errorP.textContent = 'Key cannot be empty.'; return; }
        try {
            const response = await fetch('/admin/api/users', { headers: { 'Authorization': key } });
            if (response.ok) { localStorage.setItem('admin_key', key); window.location.href = '/admin/dashboard'; } 
            else if (response.status === 401) { errorP.textContent = 'Invalid Key. Access Denied.'; } 
            else { errorP.textContent = 'An unknown error occurred.'; }
        } catch (err) { errorP.textContent = 'Failed to connect to the server.'; }
    }
...
function getAdminDashboardHTML() {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Admin Dashboard</title><style>body{background:#1a1a1a;font-family:sans-serif;color:#fff;padding:10px;}.dashboard{max-width:1000px;margin:auto;background:#2a2a2a;border-radius:15px;padding:20px;}h1,h2{text-align:center;}.create-section{background:#333;padding:15px;border-radius:10px;margin-bottom:20px;display:flex;flex-wrap:wrap;gap:10px;}input,button{padding:10px;border:none;border-radius:5px;background:#444;color:#fff;font-size:14px;}button{background:#007bff;cursor:pointer;}#new-id{flex:1 1 200px;}.user-list{overflow-x:auto;}table{width:100%;border-collapse:collapse;}th,td{padding:12px;text-align:left;border-bottom:1px solid #444;word-break:break-all;}.expired{color:#ff5252;}.active{color:#4caf50;}.actions-cell button{background:#e53935;}</style></head><body><div class="dashboard"><h1>Admin Dashboard</h1><div class="create-section"><input type="text" id="new-id" placeholder="User ID (UUID)" readonly><button onclick="generateUUID()">Generate</button><input type="date" id="exp-date"><input type="time" id="exp-time"><input type="text" id="notes" placeholder="Notes (optional)"><button onclick="createUser()">Create User</button></div><div class="user-list"><h2>User List</h2><table id="user-table"><thead><tr><th>ID</th><th>Expires</th><th>Created</th><th>Last Access</th><th>Status</th><th>Notes</th><th>Actions</th></tr></thead><tbody><tr><td colspan="7" style="text-align:center;">Loading...</td></tr></tbody></table></div></div><script>
    ...
    async function apiFetch(endpoint, options={}){const res = await fetch('/admin/api' + endpoint, {...options, headers: apiHeaders}); if(res.status === 401){alert('Unauthorized!');window.location.href='/admin/login'; throw new Error('Unauthorized');} return res;}
    function generateUUID(){document.getElementById('new-id').value = crypto.randomUUID();}
    function setupDefaultDateTime() { const now = new Date(); now.setMonth(now.getMonth() + 1); document.getElementById('exp-date').value = now.toISOString().slice(0,10); document.getElementById('exp-time').value = now.toTimeString().slice(0,5); }
    async function createUser(){const id=document.getElementById('new-id').value;const date=document.getElementById('exp-date').value;const time=document.getElementById('exp-time').value;const notes=document.getElementById('notes').value;if(!id||!date||!time)return alert('Please fill all required fields.');try {const res=await apiFetch('/users',{method:'POST',body:JSON.stringify({id,expiration_date:date,expiration_time:time,notes})});const data=await res.json();if(res.ok){alert(data.message);loadUsers();generateUUID();document.getElementById('notes').value='';}else{throw new Error(data.error||'Failed to create user');}}catch(e){alert('Error: '+e.message);}}
    async function loadUsers(){const tbody=document.getElementById('user-table').querySelector('tbody');tbody.innerHTML='<tr><td colspan="7" style="text-align:center;">Loading...</td></tr>';try{const res=await apiFetch('/users');const users=await res.json();tbody.innerHTML='';const now=Date.now()/1000;users.forEach(u=>{const isExpired=u.expiration_timestamp<=now;const statusText=isExpired?'Expired':u.status;const statusClass=isExpired?'expired':'active';const expiry=new Date(u.expiration_timestamp*1000).toLocaleString();const created=new Date(u.created_at*1000).toLocaleDateString();const lastAccess=u.last_accessed>0?new Date(u.last_accessed*1000).toLocaleString():'Never';tbody.innerHTML+=\`<tr><td>\${u.id}</td><td class="\${statusClass}">\${expiry}</td><td>\${created}</td><td>\${lastAccess}</td><td class="\${statusClass}">\${statusText}</td><td>\${u.notes||''}</td><td class="actions-cell"><button onclick="deleteUser('\${u.id}')">Delete</button></td></tr>\`;});}catch(e){tbody.innerHTML=\`<tr><td colspan="7" style="text-align:center;color:red;">Failed to load users: \${e.message}</td></tr>\`;}}
    async function deleteUser(id){if(!confirm(\`Are you sure you want to delete user \${id}?\`))return;try{const res=await apiFetch(\`/users/\${id}\`,{method:'DELETE'});const data=await res.json();if(res.ok){alert(data.message);loadUsers();}else{throw new Error(data.error||'Failed to delete user');}}catch(e){alert('Error: '+e.message);}}
    window.onload=()=>{generateUUID();setupDefaultDateTime();loadUsers();};
  </script></body></html>`;
}

function getPageCSS() {
  // This is the full CSS from your original script, which is well-styled.
  return `
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      body { font-family: var(--sans-serif); font-size: 16px; background-color: var(--background-primary); color: var(--text-primary); padding: 3rem; line-height: 1.5; -webkit-font-smoothing: antialiased; }
      ...
      .refresh-btn { position: relative; overflow: hidden; display: flex; align-items: center; gap: 4px; font-family: var(--serif); font-size: 12px; padding: 6px 12px; border-radius: 6px; color: var(--accent-secondary); background-color: var(--background-tertiary); border: 1px solid var(--border-color); cursor: pointer; transition: all var(--transition-speed) ease; }
      .refresh-btn::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(120deg, transparent, rgba(255,255,255,0.2), transparent); transform: translateX(-100%); transition: transform var(--transition-speed-long) ease; }
      .refresh-btn:hover { letter-spacing: 0.5px; font-weight: 600; background-color: #4d453e; color: var(--accent-primary); border-color: var(--border-color-hover); transform: translateY(-2px); box-shadow: 0 4px 8px var(--shadow-color); }
      .refresh-btn:hover::before { transform: translateX(100%); }
      .refresh-btn:active { transform: translateY(0px) scale(0.98); box-shadow: none; }
      ...
      .copy-buttons::before, .client-btn::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(120deg, transparent, rgba(255,255,255,0.2), transparent); transform: translateX(-100%); transition: transform var(--transition-speed-long) ease; z-index: 0; }
      ...
      .copy-icon { width: 12px; height: 12px; stroke: currentColor; position: relative; z-index: 1; }
      .client-buttons { display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 12px; margin-top: 16px; }
      .client-btn { width: 100%; background-color: var(--accent-primary); color: var(--background-tertiary); border-color: var(--accent-primary-darker); position: relative; overflow: hidden; box-shadow: 0 2px 5px rgba(0,0,0,0.15); }
      .client-btn:hover { transform: translateY(-3px); background-color: var(--accent-secondary); color: var(--button-text-primary); box-shadow: 0 5px 15px rgba(190, 155, 123, 0.5); border-color: var(--accent-secondary); }
      .client-btn:active { transform: translateY(0) scale(0.98); box-shadow: 0 2px 3px rgba(0,0,0,0.2); }
      .client-icon { width: 18px; height: 18px; border-radius: 6px; background-color: var(--background-secondary); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
	    ...
	    .button.copied { background-color: var(--status-success) !important; color: var(--background-tertiary) !important; border-color: var(--status-success) !important; }
	    .footer { text-align: center; margin-top: 20px; padding-bottom: 40px; color: var(--text-secondary); font-size: 10px; }
	    ...
	    .ip-info-item .value { font-size: 14px; color: var(--text-primary); word-break: break-all; line-height: 1.4; min-height: 20px; }
      ...
      @media (max-width: 768px) { body { padding: 20px; } .container { padding: 0 14px; } }
      @media (max-width: 480px) { body { padding: 16px; } .config-content pre {font-size: 10px;} .header h1 {font-size: 1.6rem;} .client-buttons { grid-template-columns: 1fr; } }
  `;
}
    ...
    <div class="container">
      ...
      <div class="config-card">
        ...
        <div class="ip-info-grid">
          <div class="ip-info-section">
            <div class="ip-info-header"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="2" width="14" height="20" rx="2" ry="2"></rect><line x1="12" y1="18" x2="12.01" y2="18"></line></svg><h3>Proxy Server</h3></div>
      ...
      <div class="config-card">
        ...
        <div class="client-buttons">
          <a href="${clientUrls.hiddify}" class="button client-btn"><span class="client-icon"><svg viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" /></svg></span>Import to Hiddify</a>
          <a href="${clientUrls.v2rayng}" class="button client-btn"><span class="client-icon"><svg viewBox="0 0 24 24"><path d="M12 2L4 5v6c0 5.5 3.5 10.7 8 12.3 4.5-1.6 8-6.8 8-12.3V5l-8-3z" /></svg></span>Import to V2rayNG</a>
        </div>
      </div>
      <div class="config-card">
        ...
        <div class="client-buttons">
          <a href="${clientUrls.clashMeta}" class="button client-btn"><span class="client-icon"><svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z" /></svg></span>Import to Clash Meta</a>
          <a href="${clientUrls.exclave}" class="button client-btn"><span class="client-icon"><svg viewBox="0 0 24 24"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zM9 6c0-1.66 1.34-3 3-3s3 1.34 3 3v2H9V6z" /></svg></span>Import to Exclave</a>
        </div>
      </div>
...
function getPageScript() {
  // This new client-side script is much simpler and more efficient.
  return `
      function copyToClipboard(button, text) {
        const originalHTML = button.innerHTML;
        navigator.clipboard.writeText(text).then(() => {
          button.innerHTML = \`<svg class="copy-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg> Copied!\`;
      ...
      function updateDisplay(data) {
        ...
        if (risk && typeof risk.score !== 'undefined') {
            const riskLevel = risk.risk.charAt(0).toUpperCase() + risk.risk.slice(1);
            riskText = \`\${risk.score} - \${riskLevel}\`;
