...
const Config = {
  ...
  dohUpstreamUrl: 'https://1.1.1.1/dns-query', // Added advanced feature: Configurable DoH upstream

  /**
   * @param {{ PROXYIP: string; UUID: any; SCAMALYTICS_USERNAME: any; SCAMALYTICS_API_KEY: any; SCAMALYTICS_BASEURL: any; SOCKS5: any; SOCKS5_RELAY: string; ADMIN_KEY: string; DB: D1Database; KV: KVNamespace; DOH_UPSTREAM_URL: string; }} env
   */
  fromEnv(env) {
...
/**
 * Advanced: User validation with D1 and KV caching
 * @param {string} uuid
 * @param {object} cfg
...
async function validateUser(uuid, cfg) {
  ...
  if (cachedStatus) return cachedStatus === 'valid';

  const { results } = await cfg.db.prepare('SELECT * FROM users WHERE id = ? AND status = ? AND expiration > ?')
    .bind(uuid, 'active', Date.now())
    .all();

  const isValid = results && results.length > 0;
  await cfg.kv.put(cacheKey, isValid ? 'valid' : 'invalid', { expirationTtl: 3600 }); // Cache for 1 hour
  return isValid;
}

/**
 * Advanced: Handle admin routes
...
/**
 * Advanced: Handle DoH query endpoint
 * @param {Request} request
 * @param {object} cfg
 * @returns {Promise<Response>}
 */
async function handleDoHQuery(request, cfg) {
  if (request.method !== 'POST') return new Response('Method Not Allowed', { status: 405 });
  const body = await request.arrayBuffer();
  try {
    const resp = await fetch(cfg.dohUpstreamUrl, {
      method: 'POST',
    ...
    return new Response('DoH Error', { status: 500 });
  }
}
    ...
    .pipeTo(
      new WritableStream({
        async write(chunk, controller) {
          ...
          if (isUDP) {
            if (portRemote === 53) {
              const dnsPipeline = await createDnsPipeline(webSocket, vlessResponseHeader, log, config.dohUpstreamUrl); // Advanced: Use configurable DoH
...
async function HandleTCPOutBound(
  ...
  async function retry() {
    let tcpSocket;
    try {
      // Advanced: First try direct connection
      tcpSocket = await connectAndWrite(addressRemote, portRemote);
    } catch (e) {
      log(`Direct connection failed, falling back to proxy: ${e}`);
      tcpSocket = config.enableSocks
 ...
 * @param {Function} log
 * @param {string} dohUpstreamUrl
 * @returns {Promise<{write: Function}>}
 */
async function createDnsPipeline(webSocket, vlessResponseHeader, log, dohUpstreamUrl = 'https://1.1.1.1/dns-query') {
      ...
      new WritableStream({
        async write(chunk) {
          try {
            ...
            const resp = await fetch(dohUpstreamUrl, {
              method: 'POST',
              headers: { 'content-type': 'application/dns-message' },
              body: chunk,
            });
