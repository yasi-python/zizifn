...
async function handleWebSocketRequest(request, config) {
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            ...
            HandleTCPOutBound(remoteSocketWrapper, addressType, addressRemote, portRemote, rawClientData, webSocket, log, config);
        },
        close() {
  ...
  let html = `
	...
	<body data-proxy-ip="{{PROXY_IP}}">
	  ...
	  <script>
        ...
        /**
         * [FIXED] Fetches IP information, trying ip-api.io first, then falling back to ip-api.com.
         * @param {string} ip - IP address to lookup.
         * @returns {Promise<object|null>} IP data or null on error.
         */
        async function fetchProxyGeoInfo(ip) {
            // First attempt with ip-api.io
            try {
                const responseIo = await fetch(\`https://ip-api.io/json/\${ip}\`);
                if (responseIo.ok) {
                    const dataIo = await responseIo.json();
                    if (dataIo && (dataIo.isp || dataIo.org)) {
                        return dataIo;
                    }
                }
            } catch (error) {
                console.error('Error fetching from ip-api.io:', error);
            }

            // Fallback to ip-api.com
            try {
                const responseCom = await fetch(\`https://ip-api.com/json/\${ip}?fields=status,message,country,countryCode,city,isp,org,query\`);
                if (!responseCom.ok) throw new Error(\`HTTP error! status: \${responseCom.status}\`);
                const dataCom = await responseCom.json();
                if (dataCom.status === 'success') {
                    return { // Map fields to match the expected structure
                        ip: dataCom.query,
                        country_name: dataCom.country,
                        country_code: dataCom.countryCode,
                        city: dataCom.city,
                        isp: dataCom.isp,
                        org: dataCom.org,
                    };
                } else {
                    throw new Error(dataCom.message || 'ip-api.com returned an error');
                }
            } catch (error) {
                console.error('IP API Error (ip-api.com fallback):', error);
                return null;
            }
        }
            ...
            if (proxyHostVal !== "N/A") {
                try {
                    const stripPort = (hostStr) => {
                        try {
                            // This handles IPv6 addresses in brackets like [::1]:443
                            if (hostStr.startsWith('[')) {
                                const closingBracketIndex = hostStr.indexOf(']');
                                if (closingBracketIndex > 0) return hostStr.substring(1, closingBracketIndex);
                            }
                            // Handles regular host:port or ip:port
                            const url = new URL(\`http://\${hostStr}\`);
                            return url.hostname;
                        } catch {
                            return hostStr.split(':')[0]; // Fallback for simple cases
                        }
                    };

                    const domainToResolve = stripPort(proxyHostVal);
                    let resolvedProxyIp = domainToResolve;

                    // If it's not an IP address, resolve it using Cloudflare DNS
                    if (!/^(?:[0-9]{1,3}\.){3}[0-9]{1,3}\$/.test(domainToResolve) && !domainToResolve.includes(':')) {
                        const dnsRes = await fetch(\`https://1.1.1.1/dns-query?name=\${encodeURIComponent(domainToResolve)}&type=A\`, {
                        ...
                        if (dnsRes.ok) {
                            ...
                            if (dnsData.Answer && dnsData.Answer.length > 0) {
                                ...
                                console.warn('DNS lookup had no A records for proxy domain:', domainToResolve);
                            }
                        } else {
                            console.error(\`DNS lookup failed for \${domainToResolve}: \${dnsRes.status}\`);
                        }
                    }
 ...
 */
async function HandleTCPOutBound(remoteSocketWrapper, addressType, addressRemote, portRemote, rawClientData, webSocket, log, config) {
